...
**Режим команд** — без приветствия при старте; в ЛС оператор вводит команду `/command_mode`, затем пароль (будет запрошен сообщением), задаёт HR и кандидата (`/set_hr`, `/set_candidate`), запускает опрос (`/start_questions`). После завершения опроса, комнды `/cancel` или двух не-команд подряд сессия сбрасывается, режим команд действует до остановки процесса:
